...
initialize_st_page(title="Dynamic Spread Generator", icon="🚀", initial_sidebar_state="collapsed")
...
# Assuming get_distribution function returns a list of Decimal values for spreads and amounts
...
amount_distribution = normalize(
    get_distribution(amount_dist_type, n_levels, amount_start, amount_base, amount_scaling, amount_step,
                     amount_ratio, manual_amounts))

order_amounts = [Decimal(amount_dist * total_amount_quote) for amount_dist in amount_distribution]

# Initialize lists to store values per level
take_profit_values = []
dynamic_spreads = []
break_even_values = []
spread_adjustment_factor = Decimal(0.01)  # Spread adjustment factor

for level in range(n_levels):
    if level > 0:
        # Recompute spread for current level based on the previous level's break-even price
        previous_break_even = break_even_values[level - 1]
        new_spread = previous_break_even + Decimal(spread_distribution[level])  # Dynamic spread adjustment
        dynamic_spreads.append(new_spread)
        total_amount = sum(order_amounts[:level + 1])
        break_even_values.append(Decimal(sum([spread * amount for spread, amount in zip(dynamic_spreads, order_amounts[:level + 1])]) / total_amount))
    else:
        dynamic_spreads = [Decimal(spread_distribution[0])]  # Initialize with the first spread value
        break_even_values = [Decimal(spread_distribution[0])]
...
cum_unrealized_pnl = calculate_unrealized_pnl(dynamic_spreads, break_even_values, accumulated_amount)
...
fig.add_trace(go.Scatter(x=list(range(len(dynamic_spreads))), y=dynamic_spreads, name='Spread (%)', mode='lines+markers', line=dict(width=3, color=tech_colors['spread'])), secondary_y=False)
...
for i, (spread, be_value, tp_value) in enumerate(zip(dynamic_spreads, break_even_values, take_profit_values)):
...
fig.add_annotation(
    x=max(len(dynamic_spreads), len(break_even_values)) - 1,  # Positioning the annotation to the right
...
if st.button('Export as YAML'):
    yaml_data = convert_to_yaml(spread_distribution, order_amounts)
    st.download_button(
        label="Download YAML",
        data=yaml_data,
