...
def user_inputs():
    ...
    with st.expander("SuperTrend Configuration", expanded=True):
        ...
        with c1:
            length = st.number_input("Supertrend Length", min_value=1, max_value=200, value=20)
        with c2:
            multiplier = st.number_input("Supertrend Multiplier", min_value=1.0, max_value=5.0, value=3.0)
        with c3:
            percentage_threshold = st.number_input("Percentage Threshold (%)", value=0.5) / 100
    return {
